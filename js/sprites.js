/* ==========================================================================
   HARVEST MOON: MINERAL MEADOW - PIXEL SPRITES & PROCEDURAL ART ENGINE
   ========================================================================== */

class SpriteRenderer {
  static drawPlayer(ctx, x, y, dir, isMoving, walkFrame, heldItem, isSwinging) {
    ctx.save();
    ...
    // 2. Body / Overalls
    ctx.fillStyle = '#dc2626'; // Red shirt underneath
    ctx.fillRect(-7, -2 + bob, 14, 8);
    ctx.fillStyle = '#0284c7'; // Blue Overalls
    ctx.fillRect(-5, 0 + bob, 10, 8);

    // 3. Head & Face
    ctx.fillStyle = '#fed7aa'; // Skin
    ctx.fillRect(-6, -12 + bob, 12, 10);
    ...
    ctx.fillRect(-7, -16 + bob, 14, 5); // Cap crown
    if (dir === 'down' || dir === 'left' || dir === 'right') {
      ctx.fillStyle = '#0369a1';
      ctx.fillRect(-8, -12 + bob, 16, 2); // Visor
    }

    // 5. Held Item Above Head (Harvest Moon Signature!)
    if (heldItem) {
      ctx.font = '16px sans-serif';
      ctx.textAlign = 'center';
      ctx.fillText(heldItem.icon, 0, -22 + bob);
    }
  ...
  static drawCrop(ctx, x, y, crop) {
    ...
    if (crop.stage === 0) {
      ...
      // Full Ripe Crop! (Turnip, Strawberry, Corn, etc.)
